...
const StatCard = ({
  ...
  >
    ...
    <p className="text-3xl font-bold font-futuristic text-neutral-900 dark:text-white">
      {value}
    </p>
    <p className="text-sm md:text-base text-neutral-500 uppercase tracking-widest">
      {label}
    </p>
  </motion.div>
);
...
const RepoCard = ({ repo }: { repo: Repo }) => (
  ...
  >
    <div className="flex justify-between items-start">
      ...
      <ExternalLink
        size={14}
        className="text-neutral-600 group-hover:text-sky-400 transition-colors shrink-0 mt-0.5"
      />
    </div>

    <p className="text-sm md:text-base text-neutral-500 dark:text-neutral-300 leading-relaxed line-clamp-2 flex-1">
      ...
      <div className="flex items-center gap-1.5">
        {repo.language && (
          <>
            ...
            />
            <span className="text-sm text-neutral-400 font-futuristic uppercase tracking-wider">
              {repo.language}
            </span>
          </>
        )}
...
export const Github = () => {
  ...
  return (
    <Section id="github">
      ...
      {loading && (
        <div className="flex items-center justify-center h-48 text-neutral-600 font-futuristic text-sm tracking-widest uppercase animate-pulse">
          Fetching GitHub data...
        </div>
      )}

      {error && (
        <div className="flex items-center justify-center h-48 text-neutral-600 font-futuristic text-sm tracking-widest uppercase">
      ...
      {!loading && !error && user && (
        <div className="space-y-12">
          ...
          {topLangs.length > 0 && (
            <div>
              <p className="text-sm md:text-base text-neutral-500 uppercase tracking-wider mb-4 font-futuristic">
                Language breakdown
              </p>
          ...
          {/* Top repos grid */}
          <div>
            <p className="text-sm md:text-base text-neutral-500 uppercase tracking-wider mb-6 font-futuristic">
              Featured Projects
            </p>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {FEATURED_PROJECTS.map((project, index) => (
                ...
                >
                  <h4 className="font-futuristic font-semibold text-neutral-900 dark:text-white text-base md:text-lg uppercase tracking-wider">
                    {project.name}
                  </h4>

                  <p className="text-sm md:text-base text-neutral-500 dark:text-neutral-300 leading-relaxed">
                    {project.description}
                  </p>

                  <span className="text-sm text-sky-500 uppercase tracking-wider font-medium">
                    {project.tech}
                  </span>
                </motion.a>
              ))}
